            ...
            case "import":
                return (
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M21 3H3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                );
            case "save":
                return (
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M5 4h14v12H5z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M9 4v4h6V4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                );
            case "preview":
                return (
            ...
            case "publish":
                return (
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M22 2l-7 20 1-7 7-13z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                );
            case "delete":
                return (
        ...
        <nav className="w-full bg-gray-900 text-white flex items-center justify-between px-6 py-3 z-10 flex-shrink-0">
            <div className="flex items-center gap-2">
                ...
                {leftItems?.map((item, idx) => (
                    <div key={idx} className="ml-4">
                        {item.type === "button" ? renderButton(item) : <a href={item.href} onClick={item.onClick}>{item.label}</a>}
                    </div>
                ))}
            </div>
            ...
            <div className="flex items-center gap-3">
                {centerItems?.map((item, idx) => (
                    <div key={idx}>{item.type === "button" ? renderButton(item) : <a href={item.href}>{item.label}</a>}</div>
                ))}
            </div>
